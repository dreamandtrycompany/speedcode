...
const CodeDisplayPage = () => {
  ...
  const calculateWPM = useCallback(() => {
    ...
    const totalTime = (endTime - startTimeRef.current) / 60000; // Time in minutes
    if (totalTime <= 0) return 0;
    const totalCharactersTyped = snippetScores.reduce((sum, score) => sum + score.charactersTyped, 0);
    const totalErrors = snippetScores.reduce((sum, score) => sum + score.errors, 0);
  ...
  const updateInput = useCallback((index, updater) => {
    setInputs((prev) => {
      ...
      newInputs[index] = typeof updater === 'function' ? updater(newInputs[index]) : updater;
      return newInputs;
    });
  }, []);
  ...
  const updateCurrentIndex = useCallback((index, updater) => {
    setCurrentIndices((prev) => {
      ...
      newIndices[index] = typeof updater === 'function' ? updater(newIndices[index]) : updater;
      return newIndices;
    });
  }, []);
  ...
  const updateSnippetScore = useCallback((index, charactersTyped, errors) => {
    setSnippetScores((prevScores) => {
      ...
      newScores[index] = {
        charactersTyped: Math.max(0, newScores[index].charactersTyped + charactersTyped),
  ...
  const getNextLinePosition = useCallback((currentPosition, targetCode) => {
    ...
    while (nextPosition < targetCode.length && targetCode[nextPosition] !== '\n') {
      nextPosition++;
    }
  ...
  }, [currentIndices, isActive, isCompleted, updateInput, updateCurrentIndex, updateSnippetScore, getNextLinePosition, finishGame, code]);

  const getCharClass = useCallback((charIndex) => {
    const input = inputs[0];
    const currentIndex = currentIndices[0];
    if (charIndex < currentIndex) {
      return input[charIndex] === (code?.[0]?.code || '')[charIndex] ? 'text-green-400' : 'text-red-400';
    }
    return charIndex === currentIndex ? 'text-white bg-white/20' : 'text-gray-400';
    ...
    >
      ...
      <div className="w-full max-w-6xl flex flex-col">
        <div className="mb-4 flex justify-between items-center">
          <div className="font-sans text-white text-3xl">{isCompleted ? 'COMPLETED' : <NumberFlow
                                                                                        value={timeLeft}
        ...
        {isCompleted && (
          <div className="font-sans text-white text-3xl text-center mt-8">WPM: {wpm}</div>
        )}
